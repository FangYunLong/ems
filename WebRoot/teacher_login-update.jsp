...
<%
	...
	if (action != null && action.trim().equals("update")) {
		...
		boolean result = TeacherDaoImpl.updateTeacher_login(t);
		if(result){
...
<body>
	...
	<form class="form form-horizontal" method="post"
		action="teacher_login-update.jsp?old_id=<%=t.getTid()%>">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" autocomplete="off"
					name="tname" value="<%=t.getTname()%>" disabled="disabled">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" name="tid"
					value="<%=t.getTid()%>" disabled="disabled">
			</div>
		</div>
